...
/**
 * OCEAN_UnderWriting_TC_90 class automates new PBI 18116, which holds Test
 * Cases; Test Condition Description : Vaidate new business grid
 * 
 * @author Shalu Chauhan
 */

public class OCEAN_Underwriting_TC_90 extends UnderwritingModulePages {
		...
		if (remittanceName.length() > 0) {
			...
			if (sss.size() > 1) {
				...
				if (premium.equals(finalValue)) {
					...
					HashMap<String, String> dbData = getAddedNewContractDataForContractFromDB(contractNumber);
					HashMap<String, String> dbDataMap = new HashMap<String, String>();
					dbDataMap.putAll(dbData);
					dbDataMap.put("CustomerName", dbData.get("LastName") + "," + dbData.get("FirstName"));
					dbDataMap.remove("LastName");
					dbDataMap.remove("FirstName");
					dbDataMap.put("Total", aulPrice);
					goToAccountsDetailsTab();
					goToStatementsTab();
					typeRoletypeAndRoleIdUnderStatementsTab(inputDataa[10], inputDataa[11]);
					click("clickOnCalendarIconOnStatementsTab");
					waitForSomeTime(10);
					// get month from trans date
					String transDateMonth = getMonthFromDate(dbDataMap.get("Trans_Date"));
					// click on desired month
					clickOnCalenderMonth(transDateMonth);
					clickOnViewButtonUnderStatementsTab();
					waitForSomeTime(10);
					click("IncreaseRepeat", 4);
					boolean flag = false;
					HashMap<Integer, HashMap<String, String>> uiData = getNewBusinessGridDetailsFromUI();
					for (int i = 1; i <= uiData.size(); i++) {
						if (uiData.get(i).equals(dbDataMap)) {
							flag = true;
						}
					}
					assertTrue(flag, "Contract not exist in new businesss grid.");
					logger.info("Contract exist in new businesss grid.");
