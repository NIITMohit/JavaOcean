 ...
 * @author Mohit Goel
 */
public class OCEAN_Pricing_TC_05To14_19_21_Calculation extends UnderwritingModulePages {
	 ...
	 * 
	 */
	@Test(priority = 1, groups = "calculate", dataProvider = "fetchDataForTC_05_06", dataProviderClass = UnderwritingDataProvider.class, description = "Validate premium calculation.")
	public void validatePremiumCalculationForDealerAsPrimaryAccount(String[] inputData) throws Exception {
		...
		if (sss.size() > 1) {
			...
			premiumData.put("TERM", "24/24");
			premiumData.put("COVERAGE", "POWERTRAIN");
			premiumData.put("MILEAGE", "1000");
			premiumData.put("SURCHARGESAMOUNT", "$250.00");
			premiumData.put("OPTIONSAMOUNT", "$100.00");
			premiumData.put("DEDUCTIBLEAMOUNT", "$-00.00");
			HashMap<String, String> ssss = getExceptionPremium(premiumData);
			premiumData.putAll(ssss);
		//	premiumData.putAll(enterMandatoryValuesOnContract(premiumData));
			 ...
			 * premiumData.put("SURCHARGESAMOUNT", surcharges()); if
			 * (premiumData.get("OPTIONS").toLowerCase().equals("y"))
			 * premiumData.put("OPTIONSAMOUNT", options()); if
